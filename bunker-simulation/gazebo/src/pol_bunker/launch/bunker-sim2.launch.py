...
#    get_package_share_directory("pol_bunker"), "models"
#)
os.environ["GAZEBO_MODEL_PATH"] = os.path.join(
    "/root/ros_ws/src/pol_bunker/", "models"
)
#pour empecher d'aller chercher les modèles sur gazebosim.org s'ils sont en local
...
def generate_launch_description():
    ...
    xacro_file = os.path.join(
        get_package_share_directory("pol_bunker"), "urdf/my_bunker.xacro"
    )
    xacro_file2 = os.path.join(
        get_package_share_directory("pol_bunker"), "urdf/my_bunker2.xacro"
    )
    ...
    robot_state_node2 = Node(
        ...
        output="screen",
        namespace="DT2",
        parameters=[{"robot_description": robot_desc2, "use_sim_time": use_sim_time}],
    )
